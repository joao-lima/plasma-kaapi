 ...
 *
 *  CORE_cbrdalg is a part of the bidiagonal reduction algorithm (bulgechasing).
...
#pragma weak CORE_cbrdalg = PCORE_cbrdalg
#define CORE_cbrdalg PCORE_cbrdalg
#endif
                  ...
                  int i, int j, int m, int grsiz)
{
    ...
    for (k = 0; k < grsiz; k++){
        ...
        if( myid == 1 )
           CORE_cgbelr(uplo, N, &A, V, TAU, stind, edind, eltsize);
        else if(myid%2 == 0)
           CORE_cgbrce(uplo, N, &A, V, TAU, stind, edind, eltsize);
        else /*if(myid%2 == 1)*/
           CORE_cgblrx(uplo, N, &A, V, TAU, stind, edind, eltsize);
...
{
    QUARK_Insert_Task(quark, CORE_cbrdalg_quark,   task_flags,
 ...
 **/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_cbrdalg_quark = PCORE_cbrdalg_quark
#define CORE_cbrdalg_quark PCORE_cbrdalg_quark
#endif
void CORE_cbrdalg_quark(Quark *quark)
{
    ...
    CORE_cbrdalg(uplo, N, NB, pA, V, TAU, i, j, m, grsiz);
}
