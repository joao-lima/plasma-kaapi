...
static void CORE_cbarrier_thread(const int thidx, const int thcnt);
static void CORE_camax1_thread(const PLASMA_Complex32_t localamx, 
                               const int thidx, const int thcnt, 
                               int *thwinner, PLASMA_Complex32_t *diagvalue,
 ...
 *
 *  CORE_cgetf2 computes an LU factorization of a general M-by-N matrix A
...
static void
CORE_camax1_thread(PLASMA_Complex32_t localamx, int thidx, int thcnt, int *thwinner, 
                   PLASMA_Complex32_t *diagvalue, PLASMA_Complex32_t *globalamx, int pividx, int *ipiv) {
    if (thidx == 0) {
        int i, j = 0;
        ...
        CORE_camax1buf[1] = *diagvalue; /* set the index of the winning thread */
...
CORE_cbarrier_thread(int thidx, int thcnt) {
    ...
    CORE_camax1_thread( 1.0, thidx, thcnt, &idum1, &ddum1, &ddum2, 0, &idum2 );
}

static void
CORE_cgetrf_rectil_update(const PLASMA_desc A, int *IPIV, 
...
static void 
CORE_cgetrf_rectil_rec(const PLASMA_desc A, int *IPIV, int *info, 
                       ...
                       const int ft,     const int lt)
{
    ...
    if (thidx == 0)
    {
        ...
        for(j=0;j<4;j++){
            for(i=0;i<4;i++){
                fprintf(stderr, "%e ", ((PLASMA_Complex32_t*)A(0, 0))[j*4+i]);
            }
            for(i=0;i<4;i++){
                fprintf(stderr, "%e ", ((PLASMA_Complex32_t*)A(1, 0))[j*4+i]);
            }
            fprintf(stderr, "\n");
        }
    }
    ...
    if ( width > 1 ) {
        ...
        CORE_cgetrf_rectil_rec( A, IPIV, info, pivot,
        ...
        CORE_camax1_thread( tmp1, thidx, thcnt, &thwin, 
...
int CORE_cgetrf_rectil(const PLASMA_desc A, int *IPIV, int *info)
{
    ...
    if ( A.n > minMN ) {
        CORE_cgetrf_rectil_update( A, IPIV, 
                                   0, minMN, A.n-minMN,
                                   thidx, thcnt, 
                                   ft, lt);
    }
