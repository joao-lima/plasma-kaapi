 ...
 *
 *  CORE_zhbelr is a kernel that will operate on a region (triangle) of data bounded 
 *  by st and ed. This kernel eliminate a column by an element-wise annihiliation,
 *  and for each annihiliation, it apply a left update, followed by an right 
 *  update on the diagonal 2x2 element, then it continue until finishing the 
 *  the whole column. When this is done, it take advantage that data are on 
 *  cache and will apply the right on the remaining part of this region 
 *  that has not been updated by the right yet.   
    ...
    if( uplo == PlasmaLower ) {
       ...
       for (i = ed; i >= st+1 ; i--){
          ...
          *A(i, (st-1))  = zzero;
          ...
          CORE_zlarfx2c(PlasmaLower, *V(i), *TAU(i), A(i-1, i-1), A(i, i-1), A(i, i)); 
       }
       ...
       for (i = ed; i >= st+1 ; i--){
          ...
          *A((st-1),  i) = zzero;
          ...
          CORE_zlarfx2c(PlasmaUpper, *V(i), *TAU(i), A(i-1, i-1), A(i-1, i), A(i,i));
       }
