 ...
 * @ingroup CORE_float
 *
 *  CORE_sgetf2 computes an LU factorization of a general M-by-N matrix A
...
CORE_samax1_thread(float localamx, int thidx, int thcnt, int *thwinner, 
                   float *diagvalue, float *globalamx, int pividx, int *ipiv) {
    if (thidx == 0) {
        int i, j = 0;
        ...
        CORE_samax1buf[1] = *diagvalue; /* set the index of the winning thread */
...
CORE_sbarrier_thread(int thidx, int thcnt) {
    ...
    CORE_samax1_thread( 1.0, thidx, thcnt, &idum1, &ddum1, &ddum2, 0, &idum2 );
}

static void
CORE_sgetrf_rectil_update(const PLASMA_desc A, int *IPIV, 
...
static void 
CORE_sgetrf_rectil_rec(const PLASMA_desc A, int *IPIV, int *info, 
                       float *pivot,
                       const int thidx,  const int thcnt, 
                       ...
                       const int ft,     const int lt)
{
    ...
    if (thidx == 0)
    {
        ...
        for(j=0;j<4;j++){
            for(i=0;i<4;i++){
                fprintf(stderr, "%e ", ((float*)A(0, 0))[j*4+i]);
            }
            for(i=0;i<4;i++){
                fprintf(stderr, "%e ", ((float*)A(1, 0))[j*4+i]);
            }
            fprintf(stderr, "\n");
        }
    }
    ...
    if ( width > 1 ) {
        ...
        if (thidx == 0)
        {
            ...
            { 
                ...
                if ( piv_sf ) {
                    cblas_sscal( tmpM, (pivval), L+(n1-1)*ldft, 1 );
                } else {
                    int i;
                    Atop2 = L+(n1-1)*ldft;
                ...
                tmp2    = U[n1];
                max_it  = ft;
                max_i   = cblas_isamax( tmpM, U+n1, 1 ) + n1;
                tmp1    = U[max_i];
                abstmp1 = fabsf(tmp1);
                max_i   += column;
            }
        ...
        else 
        {
            ...
            /* Scale last column of L */
            if ( piv_sf ) {
                cblas_sscal( lm, (pivval), L+(n1-1)*ld, 1 );
            } else {
                int i;
                Atop2 = L+(n1-1)*ld;
            ...
            max_it  = ft;
            max_i   = cblas_isamax( lm, L+n1*ld, 1 );
            tmp1    = L[n1*ld+max_i];
            abstmp1 = fabsf(tmp1);
        }
        ...
        for( it = ft+1; it < lt; it++)
        {
            ...
            jp = cblas_isamax( lm, L+n1*ld, 1 );
            if ( fabsf( L[n1*ld+jp] ) > abstmp1 ) {
                tmp1 = L[n1*ld+jp];
                abstmp1 = fabsf(tmp1);
                max_i  = jp;
                max_it = it;
        ...
        CORE_samax1_thread( tmp1, thidx, thcnt, &thwin, 
        ...
        if ( column == 0 )
        {
            ...
            for( it = ft+1; it < lt; it++)
            {
                Atop2= A( it, 0 );
                lm   = it == A.mt-1 ? A.m - it * A.mb : A.mb;
                jp   = cblas_isamax( lm, Atop2, 1 );
                if (  fabsf(Atop2[jp]) > abstmp1 ) {
                    tmp1 = Atop2[jp];
                    abstmp1 = fabsf(tmp1);
                    max_i  = jp;
                    max_it = it;
            ...
            if ( pivval != 0.0 ) {
                if ( thidx == 0 ) {
                    if ( fabsf(pivval) >= sfmin ) {
                         ...
                         */
                        lm = ft == A.mt-1 ? A.m - ft * A.mb : A.mb;
                        cblas_sscal( lm - column - 1, (pivval), Atop+column+1, 1 );
...
#pragma weak CORE_sgetrf_rectil = PCORE_sgetrf_rectil
#define CORE_sgetrf_rectil PCORE_sgetrf_rectil
#endif
