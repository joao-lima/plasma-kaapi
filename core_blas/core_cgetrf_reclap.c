...
static void CORE_camax1_thread(const PLASMA_Complex32_t localamx, 
                               ...
                               int *thwinner, PLASMA_Complex32_t *globalamx,
                               const int pividx, int *ipiv);

/* Laswp with inc = 1 */
static void CORE_claswap1(const int ncol, PLASMA_Complex32_t *a, const int lda, 
 ...
 *
 *  CORE_cgetrf_reclap computes an LU factorization of a general M-by-N matrix A
...
CORE_camax1_thread(PLASMA_Complex32_t localamx, int thidx, int thcnt, int *thwinner, 
                   PLASMA_Complex32_t *globalamx, int pividx, int *ipiv) {
    if (thidx == 0) {
        int i, j = 0;
...
{
    DAG_CORE_GETRF;
    QUARK_Insert_Task(quark, CORE_cgetrf_reclap_quark, task_flags,
        sizeof(int),                        &m,             VALUE,
        sizeof(int),                        &n,             VALUE,
 ...
 **/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_cgetrf_reclap_quark = PCORE_cgetrf_reclap_quark
#define CORE_cgetrf_reclap_quark PCORE_cgetrf_reclap_quark
#endif
...
static void
CORE_claswap1(const int ncol, PLASMA_Complex32_t *a, const int lda, 
              const int idxStart, const int idxMax, const int *piv) 
{
    int i, j;
