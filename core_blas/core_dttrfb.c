 ...
 * @ingroup CORE_double
 *
 *  CORE_dttrfb applies a complex upper triangular block reflector H
...
#pragma weak CORE_dttrfb = PCORE_dttrfb
    ...
    if (storev == PlasmaColumnwise) {
        ...
        if (direct == PlasmaForward) {
            ...
            if (side == PlasmaLeft) {
                ...
                /* W = A1 + W */
                for (j = 0; j < K; j++) {
                    cblas_daxpy(M1, (zone), &A1[LDA1*j], 1,
                                &WORK[LDWORK*j], 1);
                }
                ...
                for(j = 0; j < K; j++) {
                    cblas_daxpy(M1, (mzone), &WORK[LDWORK*j], 1,
                                &A1[LDA1*j], 1);
                }
