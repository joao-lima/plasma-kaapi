...
static void CORE_damax1_thread(const double localamx, 
                               ...
                               int *thwinner, double *globalamx,
                               const int pividx, int *ipiv);

/* Laswp with inc = 1 */
static void CORE_dlaswap1(const int ncol, double *a, const int lda, 
 ...
 * @ingroup CORE_double
 *
 *  CORE_dgetrf_reclap computes an LU factorization of a general M-by-N matrix A
...
CORE_damax1_thread(double localamx, int thidx, int thcnt, int *thwinner, 
                   double *globalamx, int pividx, int *ipiv) {
    if (thidx == 0) {
        int i, j = 0;
...
static inline void CORE_dgetrf_reclap_update(const int M, const int column, const int n1, const int n2,
                                             ...
                                             const int thidx, const int thcnt)
{
    ...
    if (ccnt > 0) {
        CORE_dlaswap1( ccnt, Atop2 + coff*LDA, LDA, column, n1 + column, IPIV ); /* swap to the right */
    ...
    cblas_dgemm( CblasColMajor, CblasNoTrans, CblasNoTrans, lm, n2, n1,
                 (negone), Atop+loff, LDA, Atop2 + column, LDA, (posone), Atop2+loff, LDA );
}

static void 
CORE_dgetrf_reclap_rec(const int M, const int N, 
                       ...
                       const int thidx, const int thcnt, const int column)
{
    ...
    if (N > 1) {
        ...
        CORE_dgetrf_reclap_rec( M, n1, A, LDA, IPIV, info, 
                                thidx, thcnt, column );
        if ( *info != 0 )
            return;
        
        CORE_dgetrf_reclap_update(M, column, n1, n2,
                                  A, LDA, IPIV, 
                                  thidx, thcnt);
        
        CORE_dgetrf_reclap_rec( M, n2, A, LDA, IPIV, info, 
        ...
        if (ccnt > 0) {
            CORE_dlaswap1( ccnt, Atop+coff*LDA, LDA, n1 + column, N + column, IPIV ); /* swap to the left */
        }
        ...
        jp = cblas_idamax( lm, Atop + loff, 1 );
        tmp1 = Atop[loff + jp];
        
        CORE_damax1_thread( tmp1, thidx, thcnt, &thrd, 
        ...
        if ( tmp3 != 0.0 ) {
            if ( fabs(tmp3) >= sfmin ) {
                ...
                n1 = (thidx == 0) ? 1 : 0;
                cblas_dscal( lm - n1, (tmp), Atop + loff + n1, 1 );
            } else {
                int i;
                double *Atop2;
                n1 = (thidx == 0) ? 1 : 0;
...
#pragma weak CORE_dgetrf_reclap = PCORE_dgetrf_reclap
#define CORE_dgetrf_reclap PCORE_dgetrf_reclap
#endif
...
CORE_dbarrier_thread(int thidx, int thcnt) 
{
    ...
    CORE_damax1_thread( 1.0, thidx, thcnt, &idum1, &ddum2, 0, &idum2 );
}

static void
CORE_dlaswap1(const int ncol, double *a, const int lda, 
              const int idxStart, const int idxMax, const int *piv) 
{
    int i, j;
    double tmp;
