...
RunTest(int *iparam, _PREC *dparam, real_Double_t *t_)
{
    ...
    /* } */
     ...
     /* Initialiaze Data */
    LAPACKE_dlarnv_work(1, ISEED, n*n, A);

    /* Save A and b  */
    if (check) {
        Acpy = (double *)malloc(n*n*sizeof(double));
        LAPACKE_dlacpy_work(LAPACK_COL_MAJOR, lapack_const(PlasmaUpperLower), n, n, A, n, Acpy, n);
    }

    t = -cWtime();
    PLASMA_dgecfi( n, n, A, PlasmaCM, n, 1, PlasmaCCRB, nb, nb);
    t += cWtime();
    *t_ = t;
    ...
    if (check)
      {
        dparam[TIMING_RES] = (_PREC)d_check_conversion(n, n, n, 1, nb, nb, Acpy, A, map_CM, map_CCRB);
        free(Acpy);
      }
