...
# relative to HOME
$kaapi_install_dir = ENV['HOME'] + "/install/xkaapi/default"
...
def run_kaapi_program(program, n, nb, ib, ncpu, ngpu, win_size, cache, affinity, niter, foutput, ftrace)
  ...
  if affinity == "heft"
    cmd = cmd + "KAAPI_HEFT_CALIBRATE=1 "
  end
...
#  $kaapi_log.print cmd + "\n"
  ...
  niter.times do |i|
    $kaapi_log.print "(",ncpu,",",ngpu,",",affinity,") ","(",i,"/",niter,") ",program['name']," size=",n," block=",nb," ib=",ib,"\n"
    if $kaapi_run_test == 1
      result = `#{cmd}`
...
def run_weak_scalability(program, version)
  n = 10240
#  n = 2048
  nblocks = program['nb']
  nib = program['ib']
...
#  naffinity= [ "default" ]
  ...
  ncpus.each do |ncpu|
    ...
    naffinity.each do |aff|
      output = "plasma" + "-" + program['name'] + "-" + aff + "-" + ncpu.to_s + "cpu" + ngpu.to_s + "gpu" +
	"-" + version + ".csv"
...
def run_strong_scalability(program, testname, version)
  ninput = (4096..16384).step(4096).to_a
  nblocks = program['nb']
  nib = program['ib']
  ncpu = 4
  ngpu = 8
  ...
  ninput.each do |n|
    nblocks.each do |b|
      naffinity.each do |aff|
	nib.each do |ib|
	  output = "plasma" + "-" + testname + "-" + aff + "-" + ncpu.to_s + "cpu" + ngpu.to_s + "gpu" +
	      "-" + version + ".csv"
	  trace = "perfcounter" + n.to_s + "size" + b.to_s + "nb-" + output
	  run_kaapi_program(program, testname, n, b, ib, ncpu, ngpu, win_size, cache, naffinity, $niter, output, trace)
	end
      end
    end
  end
...
def run_experiments()
  ...
  nb = [512, 1024]
  nprograms = [
    { 'file' => 'time_dpotrf_tile',	    'name' => 'PLASMA_dpotrf_Tile',	    'nb' => nb, 'ib' => ib },
    { 'file' => 'time_dgemm_tile',	    'name' => 'PLASMA_dgemm_Tile',	    'nb' => nb, 'ib' => ib },
    { 'file' => 'time_dgetrf_incpiv_tile',  'name' => 'PLASMA_dgetrf_incpiv_Tile',  'nb' => nb, 'ib' => ib },
    { 'file' => 'time_dgeqrf_tile',	    'name' => 'PLASMA_dgeqrf_Tile',	    'nb' => nb, 'ib' => ib },
    { 'file' => 'time_dgeqrfrh_tile',	    'name' => 'PLASMA_dgeqrfrh_Tile',	    'nb' => nb, 'ib' => ib }
  ]

#  p nprograms

  version = `date +%s`.chomp
  nprograms.each do |prog|
    run_weak_scalability(prog, version)
  end
end
