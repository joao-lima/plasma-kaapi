...
int main ()
{
    ...
    /* Solve the problem */
    info = PLASMA_zunmqr(PlasmaLeft, PlasmaConjTrans, M, NRHS, N, A2, LDA, T, B2, LDB);
    ...
    if ((info_solution != 0)|(info != 0))
       printf("-- Error in ZUNMQR example ! \n");
    else
       printf("-- Run of ZUNMQR example successful ! \n");
