...
int main ()
{
    ...
    info = PLASMA_dtrsm(PlasmaLeft, PlasmaLower, PlasmaNoTrans, PlasmaNonUnit, 
                        M, NRHS, (double)1.0, A2, LDA, B2, LDB);
    info = PLASMA_dormlq(PlasmaLeft, PlasmaTrans, N, NRHS, M, A2, LDA, T, B2, LDB);

    /* Check the solution */
    info_solution = check_solution(M, N, NRHS, A1, LDA, B1, B2, LDB);

    if ((info_solution != 0)|(info != 0))
       printf("-- Error in DORMLQ example ! \n");
    else
       printf("-- Run of DORMLQ example successful ! \n");
