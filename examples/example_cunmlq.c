...
int main ()
{
    ...
    /* Allocate T */
    PLASMA_Alloc_Workspace_cgelqf(M, N, &T);
    ...
    info = PLASMA_cgelqf(M, N, A2, LDA, T);

    /* Solve the problem */
    info = PLASMA_ctrsm(PlasmaLeft, PlasmaLower, PlasmaNoTrans, PlasmaNonUnit, 
                        M, NRHS, (PLASMA_Complex32_t)1.0, A2, LDA, B2, LDB);
    info = PLASMA_cunmlq(PlasmaLeft, PlasmaConjTrans, N, NRHS, M, A2, LDA, T, B2, LDB);

    /* Check the solution */
    info_solution = check_solution(M, N, NRHS, A1, LDA, B1, B2, LDB);

    if ((info_solution != 0)|(info != 0))
       printf("-- Error in CUNMLQ example ! \n");
    else
       printf("-- Run of CUNMLQ example successful ! \n");
