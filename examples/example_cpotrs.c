...
int main ()
{
    ...
    /* Plasma routines */
    info = PLASMA_cpotrf(PlasmaUpper, N, A2, LDA);
    info = PLASMA_cpotrs(PlasmaUpper, N, NRHS, A2, LDA, B2, LDB);

    /* Check the solution */
    info_solution = check_solution(N, NRHS, A1, LDA, B1, B2, LDB);

    if ((info_solution != 0)|(info != 0))
       printf("-- Error in CPOTRS example ! \n");
    else
       printf("-- Run of CPOTRS example successful ! \n");
