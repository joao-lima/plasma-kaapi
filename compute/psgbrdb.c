...
#define DEP(m)  &(DEP[m])
#define A(_m, _n)  (float *)plasma_geteltaddr(&A, (_m), (_n), eltsize)
/***************************************************************************//**
 *  Parallel Reduction from BAND Bidiagonal to the final condensed form - dynamic scheduler
 **/
void plasma_psgbrdb_quark(PLASMA_enum uplo,
                          ...
                          PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if ( NB == 1 ) {
        ...
        if(uplo==PlasmaLower){
            for (i=0; i<(MINMN-1); i++)
            {
                ...
                V             = *A((i+1), i);
                *A((i+1),  i) = zzero;
                LAPACKE_slarfg_work( 2, A(i, i), &V, 1, &TAU);
                /* apply Left*/
                TAU  = (TAU);
                ztmp = TAU*V;
                V    = (V);
    ...
    if ( MINMN <= 0 ) 
    {
        float *work, *taup, *tauq;
        int info, ldwork = N*N;
        work = (float *) plasma_shared_alloc(plasma, ldwork, PlasmaRealFloat);
        taup = (float *) plasma_shared_alloc(plasma, MINMN,  PlasmaRealFloat);
        tauq = (float *) plasma_shared_alloc(plasma, MINMN,  PlasmaRealFloat);
        
        info = LAPACKE_sgebrd_work(LAPACK_COL_MAJOR, M, N, 
    ...
    C     = (float *) plasma_shared_alloc(plasma, MINMN,   PlasmaRealFloat);
    S     = (float *) plasma_shared_alloc(plasma, MINMN,   PlasmaRealFloat);
    ...
    for (i=0; i < MINMN-1; i++) {
        ...
        if(i<(MINMN-2))
            ...
            E[i] = fabsf( *A(i, i+1));  /* last upper value is complex */
    }
    D[i] = fabsf( *A(i, i) );
#else
    for (i=0; i < MINMN-1; i++) {
