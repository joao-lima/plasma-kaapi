 ...
 *
 ******************************************************************************/
int PLASMA_csymm(PLASMA_enum side, PLASMA_enum uplo, int M, int N,
                 ...
                 PLASMA_Complex32_t beta,  PLASMA_Complex32_t *C, int LDC)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_csymm", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ( (side != PlasmaLeft) && (side != PlasmaRight) ){
        plasma_error("PLASMA_csymm", "illegal value of side");
        return -1;
    }
    if ((uplo != PlasmaLower) && (uplo != PlasmaUpper)) {
        plasma_error("PLASMA_csymm", "illegal value of uplo");
        return -2;
    }
    Am = ( side == PlasmaLeft ) ? M : N;
    if (M < 0) {
        plasma_error("PLASMA_csymm", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_csymm", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_csymm", "illegal value of LDA");
        return -7;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_csymm", "illegal value of LDB");
        return -9;
    }
    if (LDC < max(1, M)) {
        plasma_error("PLASMA_csymm", "illegal value of LDC");
        return -12;
    }
    ...
    status = plasma_tune(PLASMA_FUNC_CSYMM, M, N, 0);
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_csymm", "plasma_tune() failed");
        return status;
    }
 ...
 *
 *  PLASMA_csymm_Tile - Performs symmetric matrix multiplication.
 *  Tile equivalent of PLASMA_csymm().
 ...
 *
 ******************************************************************************/
int PLASMA_csymm_Tile(PLASMA_enum side, PLASMA_enum uplo,
 ...
 *
 *  PLASMA_csymm_Tile_Async - Performs symmetric matrix multiplication.
 *  Non-blocking equivalent of PLASMA_csymm_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_csymm_Tile_Async(PLASMA_enum side, PLASMA_enum uplo,
                            PLASMA_Complex32_t alpha, PLASMA_desc *A, PLASMA_desc *B,
                            PLASMA_Complex32_t beta,  PLASMA_desc *C,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_parallel_call_9(plasma_pcsymm,
        PLASMA_enum, side,
        PLASMA_enum, uplo,
