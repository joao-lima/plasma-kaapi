 ...
 *
 *  PLASMA_cheev - Computes all eigenvalues and, optionally,
 ...
 *
 ******************************************************************************/
int PLASMA_cheev(PLASMA_enum jobz, PLASMA_enum uplo, int N,
                 ...
                 PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_cheev", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Tune NB & IB depending on N; Set NBNB */
    status = plasma_tune(PLASMA_FUNC_CHEEV, N, N, 0);
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cheev", "plasma_tune() failed");
        return status;
    }
    ...
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_cheev", "illegal value of jobz");
        return -1;
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_cheev", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_cheev", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_cheev", "illegal value of LDA");
        return -5;
    }
    if ( (plasma_desc_check(descT) != PLASMA_SUCCESS) || 
         ( descT->m != NT*IB ) || (descT->n != NT*NB) ) {
        plasma_error("PLASMA_cheev", "invalid T descriptor");
        return -7;
    }
    if (LDQ < max(1, N)) {
        plasma_error("PLASMA_cheev", "illegal value of LDQ");
        return -9;
    }
    /* Quick return */
    if (N == 0)
    ...
    if (jobz == PlasmaVec) {
        plasma_error("PLASMA_cheev", "computing the eigenvectors is not supported in this version");
        return -1;
    }
    ...
    PLASMA_cheev_Tile_Async(jobz, uplo, &descA, W, descT, &descQ, sequence, &request);
 ...
 *
 *  PLASMA_cheev_Tile - Computes all eigenvalues and, optionally, eigenvectors of a
 ...
 *
 ******************************************************************************/
int PLASMA_cheev_Tile(PLASMA_enum jobz, PLASMA_enum uplo,
 ...
 *
 *  PLASMA_cheev_Tile_Async - Computes all eigenvalues and, optionally, eigenvectors of a
 *  complex Hermitian matrix A using a two-stage approach:
 *  First stage: reduction to band tridiagonal form;
 *  Second stage: reduction from band to tridiagonal form.
 *
 ...
 *
 ******************************************************************************/
int PLASMA_cheev_Tile_Async(PLASMA_enum jobz, PLASMA_enum uplo,
                            PLASMA_desc *A,
                            float *W,
                            PLASMA_desc *T,
                            PLASMA_desc *Q,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_dynamic_sync();
    if (jobz == PlasmaNoVec){
        INFO = LAPACKE_ssterf(descA.n, W, E);
    }else {
        INFO = LAPACKE_ssterf(descA.n, W, E);
