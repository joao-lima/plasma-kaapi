 ...
 * @ingroup float
 *
 *  PLASMA_spotrs - Solves a system of linear equations A * X = B with a symmetric positive
 *  definite (or Hermitian positive definite in the complex case) matrix A using the Cholesky
 *  factorization A = U**T*U or A = L*L**T computed by PLASMA_spotrf.
 *
 *******************************************************************************
 *
 * @param[in] uplo
 ...
 *
 ******************************************************************************/
int PLASMA_spotrs(PLASMA_enum uplo, int N, int NRHS,
                  float *A, int LDA,
                  float *B, int LDB)
{
    int NB;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_spotrs", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (uplo != PlasmaUpper && uplo != PlasmaLower) {
        plasma_error("PLASMA_spotrs", "illegal value of uplo");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_spotrs", "illegal value of N");
        return -2;
    }
    if (NRHS < 0) {
        plasma_error("PLASMA_spotrs", "illegal value of NRHS");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_spotrs", "illegal value of LDA");
        return -5;
    }
    if (LDB < max(1, N)) {
        plasma_error("PLASMA_spotrs", "illegal value of LDB");
        return -7;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_spotrs", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_spotrs_Tile_Async(uplo, &descA, &descB, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_spotrs_Tile - Solves a system of linear equations using previously
 *  computed Cholesky factorization.
 *  Tile equivalent of PLASMA_spotrs().
 ...
 *
 *  PLASMA_spotrs_Tile_Async - Solves a system of linear equations using previously
 *  computed Cholesky factorization.
 *  Non-blocking equivalent of PLASMA_spotrs_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_spotrs_Tile_Async(PLASMA_enum uplo, PLASMA_desc *A, PLASMA_desc *B,
