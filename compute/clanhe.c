 ...
 *
 ******************************************************************************/
float PLASMA_clanhe(PLASMA_enum norm, PLASMA_enum uplo, int N,
                     PLASMA_Complex32_t *A, int LDA, float *work)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_clanhe", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if ( (norm != PlasmaMaxNorm) && (norm != PlasmaOneNorm)
        && (norm != PlasmaInfNorm) && (norm != PlasmaFrobeniusNorm) ) {
        plasma_error("PLASMA_clanhe", "illegal value of norm");
        return -1;
    }
    if ( (uplo != PlasmaUpper) && (uplo != PlasmaLower) ) {
        plasma_error("PLASMA_clanhe", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_clanhe", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_clanhe", "illegal value of LDA");
        return -5;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_clanhe", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_clanhe_Tile_Async(norm, uplo, &descA, work, &value, sequence, &request);
 ...
 *
 *  PLASMA_clanhe_Tile - Tile equivalent of PLASMA_clanhe().
 ...
 *
 *  PLASMA_clanhe_Tile_Async - Non-blocking equivalent of PLASMA_clanhe_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_clanhe_Tile_Async(PLASMA_enum norm, PLASMA_enum uplo, PLASMA_desc *A, float *work, float *value,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_parallel_call_7(plasma_pclanhe,
        PLASMA_enum, norm,
        PLASMA_enum, uplo,
        PLASMA_desc, descA,
        float*, work,
