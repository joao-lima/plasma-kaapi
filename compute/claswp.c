 ...
 *
 *  PLASMA_claswp - performs a series of row interchanges on the matrix A.
 ...
 *
 ******************************************************************************/
int PLASMA_claswp(int N, PLASMA_Complex32_t *A, int LDA,
                  int K1, int K2, int *IPIV, int INCX)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_claswp", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (N < 0) {
        plasma_error("PLASMA_claswp", "illegal value of N");
        return -1;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_claswp", "illegal value of LDA");
        return -3;
    }
    ...
    status = plasma_tune(PLASMA_FUNC_CGESV, LDA, N, N);
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_claswp", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_cooplap2tile( descA, A, NB, NB, LDA, N, 0, 0, K2, N, plasma_desc_mat_free(&(descA)) );
    } else {
        plasma_ciplap2tile( descA, A, NB, NB, LDA, N, 0, 0, K2, N);
    }

    /* Call the tile interface */
    PLASMA_claswp_Tile_Async(&descA, K1, K2, IPIV, INCX, sequence, &request);
 ...
 *
 *  PLASMA_claswp_Tile - performs a series of row interchanges on the matrix A.
 *  One row interchange is initiated for each of rows K1 through K2 of A.
 *  Tile equivalent of PLASMA_claswp().
 ...
 *
 *  PLASMA_claswp_Tile_Async - performs a series of row interchanges
 *  on the matrix A.  One row interchange is initiated for each of
 *  rows K1 through K2 of A.
 *  Non-blocking equivalent of PLASMA_claswp_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_claswp_Tile_Async(PLASMA_desc *A, int K1, int K2, int *IPIV, int INCX,
