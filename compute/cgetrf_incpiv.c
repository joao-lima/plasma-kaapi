 ...
 *
 *  PLASMA_cgetrf_incpiv - Computes an LU factorization of a general M-by-N matrix A
 ...
 *          required by PLASMA_cgetrs_incpiv to solve the system of equations.
 *
 * @param[out] IPIV
 ...
 *
 ******************************************************************************/
int PLASMA_cgetrf_incpiv(int M, int N,
                  ...
                  PLASMA_Complex32_t *L, int *IPIV)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgetrf_incpiv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_cgetrf_incpiv", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_cgetrf_incpiv", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_cgetrf_incpiv", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgetrf_incpiv", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cgetrf_incpiv_Tile_Async(&descA, &descL, IPIV, sequence, &request);
 ...
 *
 *  PLASMA_cgetrf_incpiv_Tile - Computes the tile LU factorization of a matrix.
 ...
 *
 *  PLASMA_cgetrf_incpiv_Tile_Async - Computes the tile LU factorization of a matrix.
 *  Non-blocking equivalent of PLASMA_cgetrf_incpiv_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgetrf_incpiv_Tile_Async(PLASMA_desc *A, PLASMA_desc *L, int *IPIV,
