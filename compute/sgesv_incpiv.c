 ...
 * @ingroup float
 *
 *  PLASMA_sgesv_incpiv - Computes the solution to a system of linear equations A * X = B,
 ...
 *
 ******************************************************************************/
int PLASMA_sgesv_incpiv(int N, int NRHS,
                 float *A, int LDA,
                 float *L, int *IPIV,
                 float *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_sgesv_incpiv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (N < 0) {
        plasma_error("PLASMA_sgesv_incpiv", "illegal value of N");
        return -1;
    }
    if (NRHS < 0) {
        plasma_error("PLASMA_sgesv_incpiv", "illegal value of NRHS");
        return -2;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_sgesv_incpiv", "illegal value of LDA");
        return -4;
    }
    if (LDB < max(1, N)) {
        plasma_error("PLASMA_sgesv_incpiv", "illegal value of LDB");
        return -8;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgesv_incpiv", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_sgesv_incpiv_Tile_Async(&descA, &descL, IPIV, &descB, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sgesv_incpiv_Tile - Solves a system of linear equations using the tile LU factorization.
 *  Tile equivalent of PLASMA_sgetrf_incpiv().
 ...
 *
 *  PLASMA_sgesv_incpiv_Tile_Async - Solves a system of linear equations using the tile
 *  LU factorization.
 *  Non-blocking equivalent of PLASMA_sgesv_incpiv_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_sgesv_incpiv_Tile_Async(PLASMA_desc *A, PLASMA_desc *L, int *IPIV, PLASMA_desc *B,
...
*/
    ...
    plasma_parallel_call_5(plasma_psgetrf_incpiv,
        PLASMA_desc, descA,
        PLASMA_desc, descL,
        int*, IPIV,
