 ...
 * @ingroup float
 *
 *  PLASMA_sgebrd - computes the singular value decomposition (SVD) of a complex
 ...
 *
 ******************************************************************************/
int PLASMA_sgebrd(PLASMA_enum jobu, PLASMA_enum jobvt, int M, int N,
                  float *A, int LDA,
                  float *D,
                  float *E,
                  float *U, int LDU,
                  ...
                  PLASMA_desc *descT)
{
    ...
    if (jobu != PlasmaNoVec  && jobu !=PlasmaVec) {
        plasma_error("PLASMA_sgebrd", "illegal value of jobu");
        return -1;
    }
    if (jobvt != PlasmaNoVec && jobvt != PlasmaVec) {
        plasma_error("PLASMA_sgebrd", "illegal value of jobvt");
        return -2;
    }
    if (M < 0) {
    ...
    if (LDU < 1) {
        plasma_error("PLASMA_sgebrd", "illegal value of LDU");
        return -9;
    }
    if (LDVT < 1) {
        plasma_error("PLASMA_sgebrd", "illegal value of LDVT");
        return -11;
    }
    if ( (plasma_desc_check(descT) != PLASMA_SUCCESS) || 
         ( descT->m != MT*IB ) || (descT->n != NT*NB) ) {
        plasma_error("PLASMA_sgebrd", "invalid T descriptor");
        return -12;
    }
    /* Quick return */
    if (min(M, N) == 0) {
    ...
    if (jobu == PlasmaVec) {
        plasma_error("PLASMA_sgebrd", "computing the singular vectors is not supported in this version");
        return -1;
    }
    if (jobvt == PlasmaVec) {
        plasma_error("PLASMA_sgebrd", "computing the singular vectors is not supported in this version");
    ...
    PLASMA_sgebrd_Tile_Async(jobu, jobvt, &descA, D, E, &descU, &descVT, descT, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sgebrd_Tile - computes the singular value decomposition (SVD) of a complex
 *  M-by-N matrix A, optionally computing the left and/or right singular
 *  vectors.
 ...
 *
 ******************************************************************************/
int PLASMA_sgebrd_Tile(PLASMA_enum jobu, PLASMA_enum jobvt, PLASMA_desc *A,
 ...
 *
 *  PLASMA_sgebrd_Tile_Async - computes the singular value decomposition (SVD) of a complex
 *  M-by-N matrix A, optionally computing the left and/or right singular
 *  vectors.
 ...
 *
 ******************************************************************************/
int PLASMA_sgebrd_Tile_Async(PLASMA_enum jobu, PLASMA_enum jobvt, PLASMA_desc *A,
                            float *D, float *E, PLASMA_desc *U, PLASMA_desc *VT, PLASMA_desc *T,
