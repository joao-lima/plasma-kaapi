 ...
 *
 *  PLASMA_chetrd - reduces a complex Hermitian matrix A to real symmetric
 ...
 *
 * @param[in, out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_cheev
 ...
 *
 ******************************************************************************/
int PLASMA_chetrd(PLASMA_enum jobz, PLASMA_enum uplo, int N,
                 ...
                 PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_chetrd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Tune NB & IB depending on N; Set NBNB */
    status = plasma_tune(PLASMA_FUNC_CHETRD, N, N, 0);
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_chetrd", "plasma_tune() failed");
        return status;
    }
    ...
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_chetrd", "illegal value of jobz");
        return -1;
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_chetrd", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_chetrd", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_chetrd", "illegal value of LDA");
        return -5;
    }
    if ( (plasma_desc_check(descT) != PLASMA_SUCCESS) || 
         ( descT->m != NT*IB ) || (descT->n != NT*NB) ) {
        plasma_error("PLASMA_chetrd", "invalid T descriptor");
        return -8;
    }
    if (LDQ < max(1, N)) {
        plasma_error("PLASMA_chetrd", "illegal value of LDQ");
        return -10;
    }
    /* Quick return */
    if (N == 0)
    ...
    if (jobz == PlasmaVec) {
        plasma_error("PLASMA_chetrd", "computing the eigenvectors is not supported in this version");
        return -1;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        if (jobz == PlasmaVec) {
           plasma_cooplap2tile( descQ, Q, NB, NB, LDQ, N,    0, 0, N, N   , plasma_desc_mat_free(&(descQ)) );
        }
    } else {
        plasma_ciplap2tile( descA, A, NB, NB, LDA, N,    0, 0, N, N   );
        if (jobz == PlasmaVec)
           plasma_ciplap2tile( descQ, Q, NB, NB, LDQ, N,    0, 0, N, N   );
    }

    /* Call the tile interface */
    PLASMA_chetrd_Tile_Async(jobz, uplo, &descA, D, E, descT, &descQ, sequence, &request);
 ...
 * @sa PLASMA_cheev_Tile
 *
 ******************************************************************************/
int PLASMA_chetrd_Tile(PLASMA_enum jobz, PLASMA_enum uplo,
                      PLASMA_desc *A, float *D, float *E, PLASMA_desc *T, PLASMA_desc *Q)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
 ...
 *
 *  PLASMA_chetrd_Tile_Async - Computes all eigenvalues and,
 ...
 *
 ******************************************************************************/
int PLASMA_chetrd_Tile_Async(PLASMA_enum jobz, PLASMA_enum uplo,
                            PLASMA_desc *A,
                            float *D,
                            float *E,
                            PLASMA_desc *T,
