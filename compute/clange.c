 ...
 *
 * @param[in] work
 *          real array of dimension (MAX(1,LWORK)), where
 ...
 *
 ******************************************************************************/
float PLASMA_clange(PLASMA_enum norm, int M, int N,
                     PLASMA_Complex32_t *A, int LDA, float *work)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_clange", "PLASMA not initialized");
    ...
    if (M < 0) {
        plasma_error("PLASMA_clange", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_clange", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_clange", "illegal value of LDA");
        return -5;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_clange", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_clange_Tile_Async(norm, &descA, work, &value, sequence, &request);
 ...
 *
 *  PLASMA_clange_Tile - Tile equivalent of PLASMA_clange().
 ...
 *
 *  PLASMA_clange_Tile_Async - Non-blocking equivalent of PLASMA_clange_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_clange_Tile_Async(PLASMA_enum norm, PLASMA_desc *A, float *work, float *value,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_parallel_call_6(plasma_pclange,
        PLASMA_enum, norm,
        PLASMA_desc, descA,
        float*, work,
        float*, value,
