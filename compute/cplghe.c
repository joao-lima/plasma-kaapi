 ...
 *
 ******************************************************************************/
int PLASMA_cplghe( float bump, int N,
                   ...
                   unsigned long long int seed )
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cplghe", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (N < 0) {
        plasma_error("PLASMA_cplghe", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_cplghe", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (max(0, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cplghe", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cplghe_Tile_Async( bump, &descA, seed, sequence, &request );
 ...
 *
 *  PLASMA_cplghe_Tile - Generate a random hermitian matrix by tiles.
 *  Tile equivalent of PLASMA_cplghe().
 ...
 *
 ******************************************************************************/
int PLASMA_cplghe_Tile( float bump, PLASMA_desc *A,
                        unsigned long long int seed )
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
 ...
 *
 *  PLASMA_cplghe_Tile_Async - Generate a random hermitian matrix by tiles.
 *  Non-blocking equivalent of PLASMA_cplghe_Tile().
