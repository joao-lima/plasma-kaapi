                  ...
                  PLASMA_Complex32_t *T)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgeqrf", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_cgeqrf", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_cgeqrf", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_cgeqrf", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgeqrf", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cgeqrf_Tile_Async(&descA, &descT, sequence, &request);
 ...
 *
 *  PLASMA_cgeqrf_Tile - Computes the tile QR factorization of a matrix.
 ...
 *
 *  PLASMA_cgeqrf_Tile_Async - Computes the tile QR factorization of a matrix.
 *  Non-blocking equivalent of PLASMA_cgeqrf_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgeqrf_Tile_Async(PLASMA_desc *A, PLASMA_desc *T,
