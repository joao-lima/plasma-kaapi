 ...
 *
 *  plasma_cgetmi2 Implementation of inplace transposition
 *    based on the GKK algorithm by Gustavson, Karlsson, Kagstrom.
                  ...
                  PLASMA_sequence *sequence, PLASMA_request *request) 
{
    ...
    if (PLASMA_SCHEDULING == PLASMA_STATIC_SCHEDULING) {
        ...
        if (nprob > ngrp) {
            ...
            for (i=0; i< (nprob/ngrp); i++) {
                plasma_static_call_9(plasma_pcshift,
                                     int,                 me,
                                     int,                 ne,
                                     int,                 L,
    ...
    /* Dynamic scheduling */
        ...
        plasma_dynamic_call_9(plasma_pcshift,
                              int,                 me,
                              int,                 ne,
                              int,                 L,
...
void plasma_pcshift(plasma_context_t *plasma) {
    ...
    W = (PLASMA_Complex32_t*)plasma_private_alloc(plasma, L, PlasmaComplexFloat);
    ...
    for(i=0; i<nleaders; i+=3) {
        if( leaders[i+2] == locrnk ) {
            ...
            memcpy(W, &(Al[leaders[i]*L]), L*sizeof(PLASMA_Complex32_t));
            CORE_cshiftw(leaders[i], leaders[i+1], m, n, L, Al, W);
        }
            ...
            memcpy(W, &(Al[snix*L]), L*sizeof(PLASMA_Complex32_t));
            ...
            if( cl > 0 ) {
                CORE_cshiftw(snix, cl, m, n, L, Al, W);
            }
        }
    }
...
void plasma_pcshift_quark(int m, int n, int L, PLASMA_Complex32_t *A, 
                          ...
                          PLASMA_sequence *sequence, PLASMA_request *request) 
{
    ...
    for(iprob=0; iprob<nprob; iprob++) {
        ...
        QUARK_Insert_Task(plasma->quark, CORE_foo_quark, &task_flags,
                          sizeof(PLASMA_Complex32_t)*size, Al,  INOUT,
#ifdef TRACE_IPT
        ...
        for(i=0; i<nleaders; i+=3) {
            ...
            QUARK_CORE_cshift(plasma->quark, &task_flags,
                              leaders[i], m, n, L, Al);
        }
