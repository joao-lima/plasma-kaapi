                  ...
                  float *T)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_sgeqrf", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_sgeqrf", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_sgeqrf", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_sgeqrf", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgeqrf", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_sgeqrf_Tile_Async(&descA, &descT, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sgeqrf_Tile - Computes the tile QR factorization of a matrix.
 ...
 *
 *  PLASMA_sgeqrf_Tile_Async - Computes the tile QR factorization of a matrix.
 *  Non-blocking equivalent of PLASMA_sgeqrf_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_sgeqrf_Tile_Async(PLASMA_desc *A, PLASMA_desc *T,
