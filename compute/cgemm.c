 ...
 *
 ******************************************************************************/
int PLASMA_cgemm(PLASMA_enum transA, PLASMA_enum transB, int M, int N, int K,
                 ...
                 PLASMA_Complex32_t beta,  PLASMA_Complex32_t *C, int LDC)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgemm", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((transA != PlasmaNoTrans) && (transA != PlasmaTrans) && (transA != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgemm", "illegal value of transA");
        return -1;
    }
    if ((transB != PlasmaNoTrans) && (transB != PlasmaTrans) && (transB != PlasmaConjTrans)) {
        plasma_error("PLASMA_cgemm", "illegal value of transB");
        return -2;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_cgemm", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_cgemm", "illegal value of N");
        return -4;
    }
    if (K < 0) {
        plasma_error("PLASMA_cgemm", "illegal value of N");
    ...
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_cgemm", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, Bm)) {
        plasma_error("PLASMA_cgemm", "illegal value of LDB");
        return -10;
    }
    if (LDC < max(1, M)) {
        plasma_error("PLASMA_cgemm", "illegal value of LDC");
        return -13;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgemm", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        plasma_cooplap2tile( descB, B, NB, NB, LDB, Bn, 0, 0, Bm, Bn, plasma_desc_mat_free(&(descA)); plasma_desc_mat_free(&(descB)));
        plasma_cooplap2tile( descC, C, NB, NB, LDC, N,  0, 0, M,  N,  plasma_desc_mat_free(&(descA)); plasma_desc_mat_free(&(descB)); plasma_desc_mat_free(&(descC)));
    } else {
        plasma_ciplap2tile( descA, A, NB, NB, LDA, An, 0, 0, Am, An );
        plasma_ciplap2tile( descB, B, NB, NB, LDB, Bn, 0, 0, Bm, Bn );
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        plasma_ciptile2lap( descB, B, NB, NB, LDB, Bn );
 ...
 *
 *  PLASMA_cgemm_Tile - Performs matrix multiplication.
 *  Tile equivalent of PLASMA_cgemm().
 ...
 *
 ******************************************************************************/
int PLASMA_cgemm_Tile(PLASMA_enum transA, PLASMA_enum transB,
 ...
 *
 *  PLASMA_cgemm_Tile_Async - Performs matrix multiplication.
 *  Non-blocking equivalent of PLASMA_cgemm_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgemm_Tile_Async(PLASMA_enum transA, PLASMA_enum transB,
                            PLASMA_Complex32_t alpha, PLASMA_desc *A, PLASMA_desc *B,
                            PLASMA_Complex32_t beta,  PLASMA_desc *C,
