 ...
 * @ingroup float
 *
 *  PLASMA_sormlq - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 *  matrix (unitary in the complex case) defined as the product of elementary reflectors returned
 *  by PLASMA_sgelqf. Q is of order M.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *
 ******************************************************************************/
int PLASMA_sormlq(PLASMA_enum side, PLASMA_enum trans, int M, int N, int K,
                  float *A, int LDA,
                  float *T,
                  float *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_sormlq", "PLASMA not initialized");
    ...
    if ( (side != PlasmaLeft) && (side != PlasmaRight) ) {
        plasma_error("PLASMA_sormlq", "illegal value of side");
        return -1;
    }
    if ( (trans != PlasmaTrans) && (trans != PlasmaNoTrans) ){
        plasma_error("PLASMA_sormlq", "illegal value of trans");
        return -2;
    }
    if (M < 0) {
        plasma_error("PLASMA_sormlq", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_sormlq", "illegal value of N");
        return -4;
    }
    if ((K < 0) || (K > An)) {
        plasma_error("PLASMA_sormlq", "illegal value of K");
        return -5;
    }
    if (LDA < max(1, K)) {
        plasma_error("PLASMA_sormlq", "illegal value of LDA");
        return -7;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_sormlq", "illegal value of LDB");
        return -10;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sormlq", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_sooplap2tile( descA, A, NB, NB, LDA, An, 0, 0, K, An, plasma_desc_mat_free(&(descA)) );
        plasma_sooplap2tile( descB, B, NB, NB, LDB, N,  0, 0, M, N,  plasma_desc_mat_free(&(descA)); plasma_desc_mat_free(&(descB)));
    } else {
        plasma_siplap2tile( descA, A, NB, NB, LDA, An, 0, 0, K, An);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sormlq_Tile - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 *  matrix (unitary in the complex case) defined as the product of elementary reflectors returned
 *  by PLASMA_sgelqf_Tile Q is of order M.
 ...
 *
 ******************************************************************************/
int PLASMA_sormlq_Tile(PLASMA_enum side, PLASMA_enum trans,
 ...
 *
 *  Non-blocking equivalent of PLASMA_sormlq_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_sormlq_Tile_Async(PLASMA_enum side, PLASMA_enum trans,
                             PLASMA_desc *A, PLASMA_desc *T, PLASMA_desc *B,
