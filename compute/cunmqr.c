 ...
 *
 *  PLASMA_cunmqr - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 *  matrix (unitary in the complex case) defined as the product of elementary reflectors returned
 *  by PLASMA_cgeqrf. Q is of order M.
 *
 *******************************************************************************
 *
 * @param[in] side
                  ...
                  PLASMA_Complex32_t *B, int LDB)
{
    ...
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_cunmqr", "illegal value of LDB");
        return -10;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_cooplap2tile( descA, A, NB, NB, LDA, K, 0, 0, Am, K, plasma_desc_mat_free(&(descA)) );
        plasma_cooplap2tile( descB, B, NB, NB, LDB, N, 0, 0, M,  N, plasma_desc_mat_free(&(descA)); plasma_desc_mat_free(&(descB)));
    } else {
