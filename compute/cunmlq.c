 ...
 *
 *  PLASMA_cunmlq - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 *  matrix (unitary in the complex case) defined as the product of elementary reflectors returned
 *  by PLASMA_cgelqf. Q is of order M.
 *
 *******************************************************************************
 *
 * @param[in] side
                  ...
                  PLASMA_Complex32_t *B, int LDB)
{
    ...
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_cunmlq", "illegal value of LDB");
        return -10;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_cooplap2tile( descA, A, NB, NB, LDA, An, 0, 0, K, An, plasma_desc_mat_free(&(descA)) );
