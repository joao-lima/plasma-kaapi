 ...
 *
 *  PLASMA_cgeqrs - Compute a minimum-norm solution min || A*X - B || using the RQ factorization
 *  A = R*Q computed by PLASMA_cgeqrf.
 *
 *******************************************************************************
 *
 * @param[in] M
                  ...
                  PLASMA_Complex32_t *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgeqrs", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_cgeqrs", "illegal value of M");
        return -1;
    }
    if (N < 0 || N > M) {
        plasma_error("PLASMA_cgeqrs", "illegal value of N");
        return -2;
    }
    if (NRHS < 0) {
        plasma_error("PLASMA_cgeqrs", "illegal value of N");
    ...
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_cgeqrs", "illegal value of LDA");
        return -5;
    }
    if (LDB < max(1, max(1, M))) {
        plasma_error("PLASMA_cgeqrs", "illegal value of LDB");
        return -8;
    }
    /* Quick return */
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgeqrs", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        plasma_cooplap2tile( descB, B, NB, NB, LDB, NRHS, 0, 0, M, NRHS, plasma_desc_mat_free(&(descA)); plasma_desc_mat_free(&(descB)));
    } else {
    ...
    PLASMA_cgeqrs_Tile_Async(&descA, &descT, &descB, sequence, &request);
 ...
 *
 *  PLASMA_cgeqrs_Tile - Computes a minimum-norm solution using the tile QR factorization.
 *  Tile equivalent of PLASMA_cgeqrf().
 ...
 *
 *  PLASMA_cgeqrs_Tile_Async - Computes a minimum-norm solution using the tile
 *  QR factorization.
 *  Non-blocking equivalent of PLASMA_cgeqrs_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgeqrs_Tile_Async(PLASMA_desc *A, PLASMA_desc *T, PLASMA_desc *B,
