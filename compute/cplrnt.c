                   ...
                   unsigned long long int seed )
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cplrnt", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_cplrnt", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_cplrnt", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_cplrnt", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cplrnt", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cplrnt_Tile_Async( &descA, seed, sequence, &request );
 ...
 *
 *  PLASMA_cplrnt_Tile - Generate a random matrix by tiles.
 *  Tile equivalent of PLASMA_cplrnt().
 ...
 *
 *  PLASMA_cplrnt_Tile_Async - Generate a random matrix by tiles.
 *  Non-blocking equivalent of PLASMA_cplrnt_Tile().
