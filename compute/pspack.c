 ...
 * @date 2010-11-15
 *
 * @generated s Thu Sep 15 12:09:23 2011
 *
 **/
...
void plasma_pspack(plasma_context_t *plasma)
{
    ...
    W  = (float*)plasma_private_alloc(plasma, (m0*bs), PlasmaRealFloat);
    Wl = (float*)plasma_private_alloc(plasma, m1,      PlasmaRealFloat);

    /* Save leftover pieces that are otherwise going to be overwritten */
    CORE_slacpy( PlasmaUpperLower, m0, bs, &(A[(int64_t)start*m+m1]), m, W, m0 );

    /* Pack A */
    end2 = ((n-1) / size) * size + 1;
    for(i=rank+1; i<end2; i+=size) {
        memcpy( Wl, &(A[i*m]), m1*sizeof(float));
        plasma_barrier(plasma);
        memcpy( &(A[i*m1]), Wl, m1*sizeof(float));
    }
    ...
    CORE_slacpy( PlasmaUpperLower, m0, bs, W, m0, &(A[(int64_t)m1*n+start*m0]), m0 );
...
void plasma_psunpack(plasma_context_t *plasma)
{
    ...
    /* Save leftover pieces that are otherwise going to be overwritten */
    CORE_slacpy( PlasmaUpperLower, m0, bs, &(A[(int64_t)m1*n+start*m0]), m0, W, m0 );

    /* Unpack A */
    end2 = ((n-1) % size) ;
    for(i=n-1-rank; i>end2; i-=size) {
        memcpy( Wl, &(A[i*m1]), m1*sizeof(float));
        plasma_barrier(plasma);
        memcpy( &(A[i*m]), Wl, m1*sizeof(float));
    }
    ...
    /* Restore leftover pieces */
    CORE_slacpy( PlasmaUpperLower, m0, bs, W, m0, &(A[(int64_t)start*m+m1]), m );
