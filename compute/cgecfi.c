 ...
 * @date 2010-11-15
 *
 * @generated c Thu Sep 15 12:09:25 2011
 *
 **/
 ...
 *
 *  PLASMA_cgecfi convert the matrice A in place from format f_in to
 *  format f_out
 *
 *******************************************************************************
 *
 ...
 *******************************************************************************
 *
 * @sa PLASMA_cgecfi_Async
 *
 ******************************************************************************/
int PLASMA_cgecfi(int m, int n, PLASMA_Complex32_t *A,
 ...
 * @ingroup InPlaceTransformation
 *
 *  PLASMA_cgecfi_Async convert the matrice A in place from format f_in to
 *  format f_out
 *
 *******************************************************************************
 *
 ...
 *
 ******************************************************************************/
int PLASMA_cgecfi_Async(int m, int n, PLASMA_Complex32_t *A,
                        PLASMA_enum f_in,  int imb, int inb,
                        PLASMA_enum f_out, int omb, int onb,
                        PLASMA_sequence *sequence, PLASMA_request *request)
{
    plasma_context_t *plasma;
    PLASMA_Complex32_t *W = NULL;
    int im1, in1, om1, on1;
    ...
    if( f_in == PlasmaCM ) {
        if( om1 < m ) {
            plasma_static_call_6(plasma_pcpack,
                                 int,                 m,
                                 int,                 on1,
    ...
    if( (f_in  != PlasmaCM) && (f_in  != PlasmaRM) &&
        ...
        if( (imb != omb) || (inb != onb) ) {
            if( (f_in == PlasmaRRRB)  || (f_out == PlasmaRRRB ) ) {
                PLASMA_cgecfi_Async(m, n, A, f_in, imb, inb, PlasmaRM,    1,   1  , sequence, request);
                PLASMA_cgecfi_Async(m, n, A, PlasmaRM,   1,   1,   f_out, omb, onb, sequence, request);
            }
            else {
                PLASMA_cgecfi_Async(m, n, A, f_in, imb, inb, PlasmaCM,    1,   1  , sequence, request);
                PLASMA_cgecfi_Async(m, n, A, PlasmaCM,   1,   1,   f_out, omb, onb, sequence, request);
            }
            return PLASMA_SUCCESS;
        }
    }
    ...
    if( (f_out == PlasmaCM) || (f_out == PlasmaCCRB) || (f_out == PlasmaCRRB) ) 
    {
        ...
        if ( A21 > A12 ) {
            ...
            //plasma_malloc(W, max( in1, on1), PLASMA_Complex32_t);
            W = (PLASMA_Complex32_t*)malloc( max( sze1, sze2 ) * sizeof(PLASMA_Complex32_t) );
            CORE_cswpab(0, sze1, sze2, &(A[A12]), W);
            free(W);
        }
    }
    else {
        ...
        if ( A12 > A21 ) {
            ...
            //plasma_malloc(W, max( in1, on1), PLASMA_Complex32_t);
            W = (PLASMA_Complex32_t*)malloc( max( sze1, sze2 ) * sizeof(PLASMA_Complex32_t) );
            CORE_cswpab(0, sze1, sze2, &(A[A21]), W);
            free(W);
        }
    }
    ...
    if( f_out == PlasmaCM ) {
        if( im1 < m ) {
            plasma_static_call_6(plasma_pcunpack,
                                 int,                 m,
                                 int,                 in1,
                                 PLASMA_Complex32_t*, A,
                                 int,                 (m-im1),
