 ...
 * @ingroup float
 *
 *  PLASMA_spotrf - Computes the Cholesky factorization of a symmetric positive definite
 ...
 *
 ******************************************************************************/
int PLASMA_spotrf(PLASMA_enum uplo, int N,
                  float *A, int LDA)
{
    int NB;
    int status;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_spotrf", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (uplo != PlasmaUpper && uplo != PlasmaLower) {
        plasma_error("PLASMA_spotrf", "illegal value of uplo");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_spotrf", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_spotrf", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (max(N, 0) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_spotrf", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_spotrf_Tile_Async(uplo, &descA, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_spotrf_Tile - Computes the Cholesky factorization of a symmetric positive definite
 *  or Hermitian positive definite matrix.
 *  Tile equivalent of PLASMA_spotrf().
 ...
 *
 *  PLASMA_spotrf_Tile_Async - Computes the Cholesky factorization of a symmetric
 *  positive definite or Hermitian positive definite matrix.
 *  Non-blocking equivalent of PLASMA_spotrf_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_spotrf_Tile_Async(PLASMA_enum uplo, PLASMA_desc *A,
