...
static int plasmawinthread_initialized = 0;
...
static int pthread_mutex_check_for_static_initialization( pthread_mutex_t *mutex ) {
    ...
    if ( plasmawinthread_initialized == 0 ) {
        InitializeCriticalSection( &plasmawinthread_static_initializer_check_lock );
        plasmawinthread_initialized = 1;
    }
    EnterCriticalSection( &plasmawinthread_static_initializer_check_lock );
    if ( *mutex == PTHREAD_MUTEX_INITIALIZER )
        retval = pthread_mutex_init( mutex, NULL );
    LeaveCriticalSection( &plasmawinthread_static_initializer_check_lock );
    return retval;
}
