...
static int check_reduction(int, int, PLASMA_Complex32_t*, PLASMA_Complex32_t*, int, PLASMA_Complex32_t*, int, PLASMA_Complex32_t*, int, float);
...
int testing_cgesvd(int argc, char **argv)
{
    ...
    */
    ...
    PLASMA_Alloc_Workspace_cgesvd(M, N, &T);
 ...
 */
static int check_reduction(int M, int N, PLASMA_Complex32_t *A1, PLASMA_Complex32_t *A2, int LDA, 
                           PLASMA_Complex32_t *U, int LDU, PLASMA_Complex32_t *VT, int LDVT, float eps )
{
    ...
    PLASMA_Complex32_t *Aorig    = (PLASMA_Complex32_t *)malloc(M*N*sizeof(PLASMA_Complex32_t));
    PLASMA_Complex32_t *Residual = (PLASMA_Complex32_t *)malloc(M*N*sizeof(PLASMA_Complex32_t));
    PLASMA_Complex32_t *B        = (PLASMA_Complex32_t *)malloc(M*N*sizeof(PLASMA_Complex32_t));
    float *work = (float *)malloc(N*sizeof(float));

    memset((void*)B, 0, M*N*sizeof(PLASMA_Complex32_t));

    /* Rebuild the B */
    LAPACKE_clacpy_work(LAPACK_COL_MAJOR, PlasmaUpperLower, M, N, A2, LDA, B, M);
    memset((void*)Aorig, 0, M*N*sizeof(PLASMA_Complex32_t));

    /* Compute Aorig = U * B * VT */
    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, M, CBLAS_SADDR(alpha), U,     LDU, B,  M,    CBLAS_SADDR(beta), Aorig, M);
    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, N, CBLAS_SADDR(alpha), Aorig, M,   VT, LDVT, CBLAS_SADDR(beta), B,     M);
