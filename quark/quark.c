...
void QUARK_Barrier(Quark * quark)
{
    ...
    if ( quark->dot_dag_enable ) {
        ...
        fprintf(dot_dag_file, "// QUARK_Barrier reached: level=%ld \n", (long int)tasklevel );
    } else {
...
static void quark_check_and_queue_ready_task( Quark *quark, Task *task, int worker_rank )
{
    ...
    /* This case was introduced because if there is a high priority
     * small task, then other large tasks were displacing it.
     * Solution was to assign the task to this thread itself, so when
     * it returns out of the scheduling loop, it can pick up the task
     * at once  */
    if ( worker_thread_id<0 && task->priority > 0 ) { /* If priority is specified, schedule locally at once */
        int mythreadrank = QUARK_Thread_Rank( quark );
        if ( mythreadrank >= 0 ) {
            int test_thread_id = mythreadrank;
            if (( test_thread_id > 0 ) /* NOTE: Skip 0, because it is usually busy */
...
static Task *quark_work_main_loop_check_for_task( Quark *quark, Worker *worker, int worker_rank )
{
    ...
    while ( task==NULL && !worker_finalize ) {
                ...
                if ( worker_victim->executing_task==TRUE && worker_victim->ready_list_size>0 ) { /* victim has at least so many tasks */
                    ...
                    if ( task_priority_tree_node != NULL ) {
                        ...
                        if ( task_to_steal->lock_to_thread == -1 /* task not locked, so steal OK */
                             && ( task_to_steal->task_thread_count == 1 ) /* We don't steal // task */
